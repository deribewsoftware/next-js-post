...
const RightSide = () => {
  ...
  const {data:myFollowers, isLoading, isSuccess, isError}=useGetUsersDataQuery('');
  const followUsers = [
...
<div className=" space-y-4">
  ...
  <div className="space-y-4">
      {isSuccess && myFollowers.length && myFollowers.map((user:any) => (
        <FollowCard
          key={user.id}
          image={null} // Replace with user.image if image URL is available
          name={user.name}
          location={`${user.address.city}, ${user.address.street}`}
        />
      ))}
      <>
      {Array.from({ length: 6 }).map((_, index) => isLoading && <FollowSkeletonCard key={index} />)
    }
      </>
    </div>
</div>
